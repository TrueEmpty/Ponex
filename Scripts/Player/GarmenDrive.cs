...
public class GarmenDrive : MonoBehaviour
{
    ...
    void Update()
    {
        if (pG.player.CanMove && pG.player.player.superAmount >= pG.player.player.superCost)
        {
            ...
            if (Input.GetKey(pG.player.keys.right) && !Physics.Raycast(transform.position,transform.right, serachLength, layers))
            {
                moveDir += 1;
            }

            if (Input.GetKey(pG.player.keys.left) && !Physics.Raycast(transform.position, transform.right*-1, serachLength, layers))
            {
                moveDir -= 1;
            }
